...
unsigned char picking(int x, int y) {
    ...
    glPushMatrix();
    glTranslatef(0*5.0-2.5f,0,0 * 5.0-2.5f);
    drawSnowMan(1);
    glPopMatrix();

    glPushMatrix();
    glTranslatef(0*5.0-2.5f,0,1 * 5.0-2.5f);
    drawSnowMan(2);
    glPopMatrix();

    glPushMatrix();
    glTranslatef(1*5.0-2.5f,0,0 * 5.0-2.5f);
    drawSnowMan(3);
    glPopMatrix();

    glPushMatrix();
    glTranslatef(1*5.0-2.5f,0,1 * 5.0-2.5f);
    drawSnowMan(4);
    glPopMatrix();
...
void processMouseButtons(int button, int state, int xx, int yy)
{
    ...
    if (state == GLUT_DOWN)  {
        ...
        if (button == GLUT_LEFT_BUTTON)
            ...
            if (picked)
                printf("Picked Snowman number %c\n", picked-208);
            else
